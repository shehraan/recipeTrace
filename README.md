...
Built in a few hours as a full-stack prototype. Not a recipe generator. A provenance-first extraction system for knowledge that usually lives in voice, gesture, and smell.
...
Then paste a memory at `/demo/extract` or hit **Generate living recipe** on the follow-ups step (requires `OPENAI_API_KEY`).
...
3. **Follow-ups** (`/demo/followups`) — Answer targeted questions (quantities, heat, timing), then click **Generate living recipe** to run AI finalization. Unanswered questions stay visible.
4. **Living recipe** (`/demo/living`) — View the merged result with source summary (segment count, supported vs inferred steps), source/user-answer badges, and the evidence drawer.
